...
class Learner:
    ...
    def build_network(self):
        ...
        model = Sequential()
        model.add(Dense(400, activation='relu', name="dense1", input_dim=self.len_input+1))
        model.add(Dense(300, activation='relu', name="dense2"))
        model.add(Dense(300, activation='relu', name="dense3"))
        model.add(Dense(self.num_actions, activation='linear', name="dense4"))
    ...
    def run(self):#, server):
        ...
        while self.no_added_count < self.max_queue_no_added:
            ...
            for data in minibatch:
                ...
                if self.lstm:
                    self.total_q_max += np.max(self.q_values.eval(feed_dict={
                        self.chart: [np.float32(data[0])[:-1].reshape(len(data[0])-1,1)],
                        self.posi: np.float32(data[0][-1]).reshape(1,1)}))
                else:
            ...
            if self.lstm:
                ...
                actions = np.argmax(self.q_values.eval(feed_dict={
                    self.chart: np.float32(np.array(state_batch)[:,:-1].reshape(len(state_batch),len(state_batch[0])-1,1)),
                    self.posi: np.float32(np.array(state_batch)[:,-1].reshape(len(state_batch),1))}),
                    axis=1)
